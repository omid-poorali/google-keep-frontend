...
export interface BodyScrollOptions {
  ...
  let previousBodyPosition: { position: any; top: any; left: any; } | undefined;
  ...
  const preventDefault = (rawEvent: HandleScrollEvent): boolean => {
    ...
    if (allowTouchMove(e.target as EventTarget)) {
      return true;
    }
  ...
  const setOverflowHidden = (options?: BodyScrollOptions) => {
    ...
    if (previousBodyPaddingRight === undefined) {
      const reserveScrollBarGap = Boolean(options) && options?.reserveScrollBarGap === true;
